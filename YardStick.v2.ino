...
void loop() 
{
  ...
  int LosantConnectionResult = Losant_setup();
  if (LosantConnectionResult > 0)
  {
    //----------------------------------------------------//
    DBG("DBG: loop:: Starting project specific Setup.\n"); //
    dht.begin();      // start the DHT object
    LoadAnalogPinParameters(); // Load saved soil humidity sensor parameters if they exist
    VERBOSE("VRB: loop:: Setup done.\n");
    //----------------------------------------------------//
    VERBOSE("VRB: loop:: Starting the active(loop) part.\n");
    ...
    INFO("INF: loop:: Data sampled & sent to Losant!\n");
    device.disconnect();
  }
  else
    ERR("ERR: loop:: Failed to connect to Losant, skipping active part.\n");
    
  VERBOSE("VRB: loop:: ended, starting loop delay of %d seconds.\n", REPORT_INTERVAL);
